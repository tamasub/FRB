...
> **Expected Diffを直接手書きするのではなく、Inputをコピーした「仮Expected Output」にテスト条件だけを反映し、Inputとの差分からExpected Diffを機械生成すること。**
...
2. Inputをコピーして仮Expected Outputを作る。

3. テスト条件で期待する変更だけを仮Expected Outputへ反映する。

4. Input vs 仮Expected OutputからExpected Diffを機械生成する。

5. APP実行後は Input vs Actual OutputからActual Diffを生成する。

6. Expected Diff vs Actual Diffが一致すればPASS。

7. 変更指定されていない箇所はRunnerがInput値を継承して保証する。

8. 波及変更を生成できない場合は、
   「Runnerに何の知恵が足りないか？」を考える。

9. 必要ならAPP側を、責務単位で実行・観測可能な構造へ変更する。
```
...
> **Expected Diff方式とは、Inputを正本として仮Expected Outputを機械生成し、「期待する変化」だけを差分として取り出し、APPが実際に起こした変化との差分比較によって保証する方式である。**

> 期待値差分とは、品質保証済みのGit Diff の様なツール出力を期待値として扱うことを前提とした手法である。
