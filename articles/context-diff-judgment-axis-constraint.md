...
まさにその通りで、だからこそ **発見** だったと思っている。実験設計としては「変数の汚染」で失敗だった。でもプロンプト設計の実務として見れば、これは使える技術でもある。文章で長々と制約を書くより、出力させるJSONのキー名だけで、ある程度の制約をAIへ課せる。それは確実で、トークンの消費も少なく済む。狙わずに起きればバグ、**狙って使えばハック** 。今回はそのどちらの顔も見た、ということだと思う。
