...
前回の記事では、AI協働で何でも「文脈」と呼んでしまうと、判断に必要な情報がぼやけるという話を書いた。

そこで、少なくとも次の情報は分けて扱うことにした。
...
1. 制約違反の候補を落とす
2. 残った候補を判断軸で比較する
3. 採用した結果を、責務として誰が保証するかを決める
```

という判断の流れが見え始めた。

さらに、個別の判断は判断ログへ残し、そこから再利用できる判断軸・制約・責務を蒸留していく。

前回の記事の最後では、この考え方を備忘録として、ひとまず **「AI協働 思考フレームワーク」** と置いた。

しかし、書きながら一つ思った。

> これ、もう備忘録ではなく、AI協働における「判断構造」として独立させてもいいのではないか。
...
の4つを、AI協働における判断構造として整理し直す。

そして、この判断構造モデルに名前を付けた。
...
前記事で備忘録的に置いていた構造を、ここで正式に整理する。

AI協働では、すべてを一つの巨大な「プロンプト」や「文脈」に押し込むのではなく、
...
| 領域 | 役割 | 一言でいうと |
| --- | --- | --- |
| **CORA Model** | 再利用する判断情報 | 判断の型 |
| **判断ログ** | 個別の決定と理由を残す | 判例 |
| **思考ログ** | まだ収束していない違和感や仮説を残す | 鉱脈 |
...
flowchart TB
    ...
    T[思考ログ<br/>違和感・仮説・試行錯誤]
    D[判断ログ<br/>判断・判断理由]
    C["CORA Model<br/>文脈 / 判断軸 / 制約 / 責務"]
    E[実行・比較・差分]
    A{人間承認}
...
これは今後も越えてはいけない境界ではないか
→ Restrictions

これは誰かが継続的に保証すべきことではないか
→ Accountability
...
このフレームワークの中で、**CORA Modelは「判断に使う再利用可能な構造」**を担当する。

ここから、そのCORA Model自体を整理する。
...
## CORAの特徴 — 責務とExpectedを切り離さない

CORA Modelでは、**Accountability（責務）とExpected（期待される結果）を強く結び付ける。**

これは、CORAの中でも特に重要な考え方である。
...
ここでいうExpectedは、単なるテスト用の期待値ではない。

> **Expected = 責務が成立した世界の記述**
...
```text
責務:
検索Runtimeは、検索条件に一致する行だけを返すことを保証する。

Expected:
結果には検索条件に一致する行だけが存在し、
不一致の行は存在しない。
```
...
人間が見るのは、巨大な成果物そのものではない。

> **責務として約束した世界と、実際に生成された世界との差分**

を見る。

これは、AI承認駆動開発の考え方とも直接つながる。

AIが大量のコードや成果物を生成する世界では、人間がすべてを読むことには限界がある。
...
まで構造化されていれば、人間はその差分を承認できる。

つまりCORAにおけるAccountabilityは、単なる「役割・責任」の分類ではない。

> **責務をExpectedへ接続し、人間が承認可能な状態まで持っていくための構造**

として扱う。

現時点では、この **「責務とExpectedを結合する」** という考え方が、CORA Modelの最も特徴的な部分の一つだと考えている。

（その詳細は、今後、AI承認駆動開発理論の中で、紹介していけるよう検証中）
...
追伸：
　”CORA Model”は、本日(2026/8/28) ChatGPTと私とで命名したモデル名称であること、念のため申し添えておく💦
