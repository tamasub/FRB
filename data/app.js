...
const attackMarks = []; // {t, score, label}
...
const BANDS = [
  ...
  { key:'b3', label:'250-350', f0:250, f1:350 },
  { key:'b4', label:'350-450', f0:350, f1:450 },
...
const BANDS_INVESTIGATE = [
  ...
  // ★ここから上帯域追加（既存に影響なし）
  // { key:'b7',   label:'280-320',  f0:280, f1:320 },
  // { key:'b8',   label:'320-380',  f0:320, f1:380 },
  // { key:'b9',   label:'380-450',  f0:380, f1:450 },
  // { key:'b10',  label:'450-500',  f0:450, f1:500 },
];
...
const BANDS_INVESTIGATE_LEGEND = [
  ...
  ['b2_5', '80-100Hz',  '中低域ブリッジ'],

  ['b3',   '100-120Hz', '翻訳帯域'],
  ['b4',   '120-160Hz', '中域キャラクター'],

  ['b4_5', '160-220Hz', '中域→高域ブリッジ'],

  ['b5',   '220-240Hz', '倍音ジンジン帯'],
  ['b6',   '240-280Hz', '高域余韻帯'],

  // ★追加
  // ['b7',   '280-320Hz', '中高域入口'],
  // ['b8',   '320-380Hz', '高域ブリッジ'],
  // ['b9',   '380-450Hz', '高域キャラクター'],
  // ['b10',  '450-500Hz', 'ぷるぷる余韻帯'],
...
const BAND_COLORS_INVESTIGATE = [
  ...
  // ★追加（自然な延長）
  // '#ef5350', // 280-320（赤寄り維持）
  // '#ab47bc', // 320-380（紫：高域ブリッジ感）
  // '#7e57c2', // 380-450（深紫）
...
// ===== FRB Piano Roll Master Volume Multiplier =====
// Piano Roll の Play / WAV / MIDI 全トラックへ同じ倍率を掛ける。
// デフォルトは比較実験で使いやすいように 10 倍。
let PIANO_ROLL_VOLUME_MULTIPLIER = 10;
...
const pianoRollVolumeMultiplierEl = document.getElementById('pianoRollVolumeMultiplier');
const pianoRollVolumeMultiplierLabel = document.getElementById('pianoRollVolumeMultiplierLabel');

function getPianoRollVolumeMultiplier(){
  const v = Number(pianoRollVolumeMultiplierEl?.value);
  return Number.isFinite(v) && v > 0 ? v : PIANO_ROLL_VOLUME_MULTIPLIER;
}

function updatePianoRollVolumeMultiplierLabel(){
  PIANO_ROLL_VOLUME_MULTIPLIER = getPianoRollVolumeMultiplier();
  if (pianoRollVolumeMultiplierLabel) {
    pianoRollVolumeMultiplierLabel.textContent = `${PIANO_ROLL_VOLUME_MULTIPLIER.toFixed(1).replace(/\.0$/, '')}x`;
  }
}
...
//  buildBandRows('bandNowM',  'barM',   'txtM',   'bandFillM');
  ...
  if (pianoRollVolumeMultiplierEl) {
    updatePianoRollVolumeMultiplierLabel();
    pianoRollVolumeMultiplierEl.addEventListener('input', () => {
      updatePianoRollVolumeMultiplierLabel();
    });
    pianoRollVolumeMultiplierEl.addEventListener('change', () => {
      updatePianoRollVolumeMultiplierLabel();
    });
  }
...
function fitCanvasEl(el, minW = 300, minH = 60){
  const dpr = window.devicePixelRatio || 1;
  const rect = el.getBoundingClientRect();
  el.width  = Math.max(minW, Math.floor(rect.width * dpr));
  el.height = Math.max(minH, Math.floor(rect.height * dpr));
}
...
function fitAll(){
  ...
  if (gMelodyAmp && melodyAmpCanvas) {
    gMelodyAmp.clearRect(0, 0, melodyAmpCanvas.width, melodyAmpCanvas.height);
  }
...
function pushAttackMark(t, score, label){
  attackMarks.push({ t, score, label });
  if (attackMarks.length > 200) {
    attackMarks.shift();
  }
}
...
function drawBandTimeline(){
  ...
  const W = tsBandCanvas.width;
  const H = tsBandCanvas.height;
  gTB.clearRect(0, 0, W, H);
  ...
  gTB.fillStyle = '#fcfcfc';
  gTB.fillRect(0, 0, W, H);

  gTB.strokeStyle = '#d4d4d4';
  gTB.lineWidth = 1;
  gTB.beginPath();
  gTB.moveTo(chartLeft, chartTop);
  gTB.lineTo(chartLeft, chartBottom);
  gTB.lineTo(chartRight, chartBottom);
  gTB.stroke();

  if (!seriesSrc || seriesSrc.length < 2) {
    gTB.fillStyle = '#999';
    gTB.font = '12px sans-serif';
    gTB.fillText('no band timeline yet', chartLeft + 10, chartTop + 18);
    return;
  }
...
function drawBandFluxTimeline(){
  if (!tsBandFluxCanvas || !gTBF) return;

  const W = tsBandFluxCanvas.width;
  const H = tsBandFluxCanvas.height;
  gTBF.clearRect(0, 0, W, H);
  ...
  gTBF.fillStyle = '#fcfcfc';
  gTBF.fillRect(0, 0, W, H);

  gTBF.strokeStyle = '#d4d4d4';
  gTBF.lineWidth = 1;
  gTBF.beginPath();
  gTBF.moveTo(chartLeft, chartTop);
  gTBF.lineTo(chartLeft, chartBottom);
  gTBF.lineTo(chartRight, chartBottom);
  gTBF.stroke();

  if (!seriesSrc || seriesSrc.length < 2) return;
  ...
  gTBF.font = '10px sans-serif';
  gTBF.textAlign = 'right';
  gTBF.textBaseline = 'middle';

  BANDS.forEach((band, idx) => {
    const y0 = laneTop(idx);
    const y1 = laneBottom(idx);

    gTBF.strokeStyle = '#ececec';
    gTBF.lineWidth = 1;
    gTBF.beginPath();
    gTBF.moveTo(chartLeft, y0);
    gTBF.lineTo(chartRight, y0);
    gTBF.stroke();

    gTBF.beginPath();
    gTBF.moveTo(chartLeft, y1);
    gTBF.lineTo(chartRight, y1);
    gTBF.stroke();

    gTBF.fillStyle = '#666';
    gTBF.fillText(band.label, chartLeft - 8, (y0 + y1) / 2);
  });

  BANDS.forEach((band, idx) => {
    gTBF.strokeStyle = colors[idx % colors.length];
    gTBF.lineWidth = 2;
    gTBF.beginPath();

    let started = false;
    for (const p of visible) {
      const raw = Number(p.fluxBandA?.[band.key]) || 0;
      const ratio = Math.pow(raw / (globalMax * 1.05), 0.7);
    ...
    if (started) gTBF.stroke();
  });

  gTBF.font = '11px sans-serif';
  gTBF.textAlign = 'left';
...
function drawFRBPianoRoll(data = window.FRB_PIANO_ROLL_LAST){
  ...
  for (let i = 0; i < frames.length; i++) {
    ...
    // 主旋律は元の低/中/高ノートから削らず、黄色で別レイヤー表示。
    if (fr.melody) {
...
// 主旋律として選ばれたピークの「元振幅」をリアルタイムで表示する。
// 表示スケールは見やすさのために自動だが、値そのものは加工しない。
let melodyAmpRenderPending = false;
let investigateReplayMelodyAmp = [];
let liveMelodyAmpHistory = [];
let liveMelodyAmpHz = NaN;
const LIVE_MELODY_AMP_KEEP_SEC = 90;

function getMelodyAmpCanvas(){
  return document.getElementById('melodyAmpCanvas');
}

function setMelodyAmpStatus(text){
  const el = document.getElementById('melodyAmpStatus');
  if (el) el.textContent = text;
}

function makeMelodyAmpPoint(frame, melodyPeak){
  const t = Number(frame?.t) || 0;
  if (!melodyPeak || !Number.isFinite(Number(melodyPeak.hz))) {
    return { t, hz: 0, note: '', mag: 0, midi: null, hasMelody: false };
  }
  const pitch = makeReplayPitchInfo(melodyPeak.hz);
  return {
    t,
    hz: Number(melodyPeak.hz) || 0,
    playHz: pitch.playHz,
    midi: pitch.midi,
    note: pitch.note,
    mag: Math.max(0, Number(melodyPeak.mag) || 0),
...
function pushLiveMelodyAmpFrame(frame){
  ...
  const lastT = Number(point.t) || 0;
  while (liveMelodyAmpHistory.length &&
         (lastT - (Number(liveMelodyAmpHistory[0]?.t) || 0)) > LIVE_MELODY_AMP_KEEP_SEC) {
    liveMelodyAmpHistory.shift();
  }
...
function rebuildMelodyAmpFromLoadedSeries(){
  ...
  const src = Array.isArray(tsBuf) ? tsBuf : [];
  for (const frame of src) {
    if (!Array.isArray(frame?.rawTopPeaksA) || !frame.rawTopPeaksA.length) continue;
    const melodyPick = pickMainMelodyPeak(frame, liveMelodyAmpHz);
    liveMelodyAmpHz = melodyPick.hz;
    const point = makeMelodyAmpPoint(frame, melodyPick.peak);
    point.mode = 'loaded';
  ...
  window.FRB_MELODY_AMP_LAST = liveMelodyAmpHistory;
  window.FRB_MELODY_AMP_MODE = 'loaded';
  requestDrawMainMelodyAmp();
}

function drawMainMelodyAmp(data = window.FRB_MELODY_AMP_LAST){
  const canvas = getMelodyAmpCanvas();
  if (!canvas) return;
  ...
  if (!points.length) {
    ctx.fillStyle = 'rgba(235,245,255,0.72)';
    ctx.textAlign = 'left';
    ctx.fillText('計測中に、主旋律候補ピークの元振幅がここにリアルタイム表示されます', left + 10, top + 22);
    setMelodyAmpStatus('idle');
    return;
  }

  const t0 = Number(points[0]?.t) || 0;
  const t1 = Number(points[points.length - 1]?.t) || (t0 + points.length);
  const spanT = Math.max(1e-6, t1 - t0);

  let maxMag = 1e-9;
  for (const p of points) {
    maxMag = Math.max(maxMag, Number(p?.mag) || 0);
  }

  function xMap(t){
    return left + ((Number(t) - t0) / spanT) * plotW;
  }
  function yMap(v){
    return bottom - (Math.max(0, Number(v) || 0) / maxMag) * plotH;
  }

  // grid + raw scale labels
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    ctx.lineTo(right, y);
    ctx.stroke();
    const v = maxMag - maxMag * i / 4;
    ctx.fillText(v.toFixed(0), left - 8, y);
  }
  ...
  // fill dots every few frames for readability
  ...
  const last = [...points].reverse().find(p => p?.hasMelody) || points[points.length - 1];
  const lastMag = Number(last?.mag) || 0;
  const lastHz = Number(last?.hz) || 0;
  const lastNote = last?.note || '';

  ctx.fillStyle = 'rgba(255,224,102,0.95)';
  ctx.textAlign = 'left';
  ctx.textBaseline = 'top';
  ctx.fillText(`Live Melody Candidate Amp(raw): ${lastMag.toFixed(0)}`, left + 8, top + 8);
  ctx.fillStyle = 'rgba(235,245,255,0.74)';
  ctx.fillText(`Hz: ${lastHz.toFixed(1)} ${lastNote ? '/ ' + lastNote : ''}`, left + 8, top + 24);

  ctx.textBaseline = 'alphabetic';
  ctx.textAlign = 'right';
  ctx.fillText(`scale max(raw): ${maxMag.toFixed(0)} / frames: ${points.length}`, right, H - 8);

  const mode = window.FRB_MELODY_AMP_MODE || 'live';
  setMelodyAmpStatus(`${mode} amp ${lastMag.toFixed(0)} / ${lastHz.toFixed(1)}Hz${lastNote ? ' ' + lastNote : ''}`);
}

function requestDrawMainMelodyAmp(){
  if (melodyAmpRenderPending) return;
  melodyAmpRenderPending = true;
  requestAnimationFrame(() => {
    melodyAmpRenderPending = false;
    drawMainMelodyAmp();
  });
}

window.addEventListener('resize', () => requestDrawMainMelodyAmp());
...
let investigateReplayCtx = null;
let investigateReplayOscs = [];
let investigateReplayGains = [];
let investigateReplayTimer = null;
...
function ensureInvestigateReplayAudio(){
  ...
  if (investigateReplayCtx.state === 'suspended') {
    investigateReplayCtx.resume();
  }
}

function getInvestigateReplayFreq(band){
  // BandTimeline Investigate の各周波数帯の中央値Hzを鳴らす。
  // 例: 20-40Hz -> 30Hz, 80-100Hz -> 90Hz
  const f0 = Number(band?.f0) || 0;
  const f1 = Number(band?.f1) || f0;
  return Math.max(1, (f0 + f1) / 2);
}

function getInvestigateBandValue(frame, band){
  const map = frame?.bandAInvestigate;
  if (!map) return 0;
  return Math.max(0, Number(map[band.key]) || 0);
}
...
function pickFrb123Peaks(frame){
  ...
  // backing構成：低域2音 / 中域2音 / 高域3音 = 7音。
  // 主旋律はここから削らず、別トラック・別表示として複製する。
  const bands = [
...
function startInvestigateReplay(){
  ...
  // Live Melody Candidate Amp は検証用のログ表示として保持する。
  // Load後のログ情報を、Play Investigate でクリア・再構築しない。
  requestDrawFRBPianoRoll();
  ...
  for (let i = 0; i < 7; i++) {
    ...
    osc.connect(gain);
    gain.connect(investigateReplayCtx.destination);
    osc.start();

    investigateReplayOscs.push(osc);
  ...
  function step(){
    ...
    if (investigateReplayIndex >= investigateReplayFrames.length) {
      stopInvestigateReplay(true);
      return;
    }
    ...
    window.FRB_PIANO_ROLL_LAST = investigateReplayPianoRoll;
    // 再生中も Live Melody Candidate Amp は触らない。
    // MIDI音量検証のため、Load時に復元した主旋律候補Ampを表示したままにする。
    requestDrawFRBPianoRoll();
    ...
    for (let i = 0; i < 7; i++) {
      ...
      // 再生用ゲイン。可聴化用の正規化で、MIDI Velocity/主旋律Amp表示には使わない。
      const norm = Math.sqrt(mag / frameMax);
      const REPLAY_GAIN = 0.6;
      const gainValue = Math.min(0.9, norm * baseVol * REPLAY_GAIN * getPianoRollVolumeMultiplier());
    ...
    const melodyPoint = investigateReplayMelodyAmp[investigateReplayMelodyAmp.length - 1];
    investigateReplayIndex++;
    setInvestigateReplayStatus(
      melodyPick.peak
        ? `playing FRB 2-2-3 ${investigateReplayIndex}/${investigateReplayFrames.length} / melody ${Number(melodyPick.peak.hz).toFixed(1)}Hz amp ${Number(melodyPick.peak.mag).toFixed(0)} / voices ${peaks.length}`
    ...
    const next = investigateReplayFrames[investigateReplayIndex];
    let delayMs = 80;

    if (cur && next && Number.isFinite(cur.t) && Number.isFinite(next.t)) {
      delayMs = Math.max(20, Math.min(250, (next.t - cur.t) * 1000));
    }

    investigateReplayTimer = setTimeout(step, delayMs);
  }
...
function stopInvestigateReplay(finished = false){
  ...
  if (investigateReplayTimer) {
    clearTimeout(investigateReplayTimer);
    investigateReplayTimer = null;
  }

  const now = investigateReplayCtx?.currentTime || 0;
  for (const gain of investigateReplayGains) {
    try {
      gain.gain.setTargetAtTime(0, now, 0.02);
    } catch(e) {}
  }

  for (const osc of investigateReplayOscs) {
  ...
  requestDrawFRBPianoRoll();
  // 停止時も Live Melody Candidate Amp は触らない。
}
...
function exportWavFromNotes() {
  ...
  for (let f = 0; f < data.length; f++) {
    ...
    for (let i = 0; i < frameSamples && pos < samples.length; i++, pos++) {
      ...
      samples[pos] = v * masterGain * getPianoRollVolumeMultiplier();
    }
  }
  ...
  // 倍率アップ後、クリップしそうな時だけ軽く抑える。
  // 常時正規化すると倍率指定の意味が消えるため、peak <= 0.95 の時はそのまま出す。
  let peak = 0;
...
// ===== FRB MIDI Export (self-contained SMF writer) =====
...
function writeU16BE(v){ return [(v >> 8) & 0xff, v & 0xff]; }
function writeU32BE(v){ return [(v >>> 24) & 0xff, (v >>> 16) & 0xff, (v >>> 8) & 0xff, v & 0xff]; }
...
function secondsToMidiTick(sec){
  ...
  return Math.round(s * 2 * FRB_MIDI_TICKS_PER_QN); // 120 BPM = 2 quarter notes/sec
}

function noteObjToSemitoneMidi(note){
  if (note && Number.isFinite(Number(note.midi))) return Math.round(Number(note.midi));
  if (note && Number.isFinite(Number(note.rawHz))) {
...
function velocityFromMag(mag, hz){
  const m = Math.max(0, Number(mag) || 0) * getPianoRollVolumeMultiplier();
...
function makeMidiTrack(trackName, channel, program, noteEvents, maxMag, includeTempo = false, trackMemoText = ''){
  ...
  for (const ev of abs) {
    ...
    events.push([...vlq(delta), ...ev.bytes]);
    lastTick = ev.tick;
  }
...
function exportMidiFromPianoRoll(){
  ...
  const headerBody = [
    ...writeU16BE(1),
    ...writeU16BE(tracks.length),
    ...writeU16BE(FRB_MIDI_TICKS_PER_QN),
  ];
  const bytes = [
    ...strBytes('MThd'), ...writeU32BE(headerBody.length), ...headerBody,
    ...tracks.flat(),
  ];
