...
if (
    -not (Test-Path -LiteralPath (Join-Path $root "firmware") -PathType Container) -or
    -not (Test-Path -LiteralPath (Join-Path $root "data") -PathType Container) 
) {
...
$zipName = "FRB_FFT_App$timestamp.zip"
...
try {
    Write-Host ""
    Write-Host "FRB FFT package zip create start"
