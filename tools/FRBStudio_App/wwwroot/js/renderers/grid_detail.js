...
function renderGrid() {
  if (typeof refreshDefinitionTargetCaptionDerivedProperties === 'function') {
    refreshDefinitionTargetCaptionDerivedProperties(viewDef, sourceData);
  }
  if (typeof updateViewDefMaintenanceMoveButtons === 'function') {
    updateViewDefMaintenanceMoveButtons();
  }
  const gd = gridDef();
...
function currentViewDefMaintenanceRowKey() {
  if (typeof isViewDefMaintenanceDocument !== 'function' || !isViewDefMaintenanceDocument(sourceData)) return '';
  if (!Array.isArray(currentRows) || selectedIndex < 0 || !currentRows[selectedIndex]) return '';
  return String(currentRows[selectedIndex]?.__maintenance_key ?? '');
}

function viewDefMaintenanceMoveNeighborIndex(index, delta) {
  if (!Array.isArray(currentRows) || index < 0 || ![-1, 1].includes(Number(delta))) return -1;
  const ref = String(currentRows[index]?.__maintenance_section_ref ?? '');
  if (!ref) return -1;
  let cursor = index + Number(delta);
  while (cursor >= 0 && cursor < currentRows.length) {
    if (String(currentRows[cursor]?.__maintenance_section_ref ?? '') === ref) return cursor;
    cursor += Number(delta);
  }
  return -1;
}

function updateViewDefMaintenanceMoveButtons() {
  const up = $('viewDefMoveUpBtn');
  const down = $('viewDefMoveDownBtn');
  if (!up || !down) return;

  const active = typeof isViewDefMaintenanceDocument === 'function' && isViewDefMaintenanceDocument(sourceData);
  up.classList.toggle('hidden', !active);
  down.classList.toggle('hidden', !active);
  if (!active) {
    up.disabled = true;
    down.disabled = true;
    return;
  ...
  const sorted = Boolean(sortState?.field);
  const index = selectedIndex;
  up.disabled = sorted || index < 0 || viewDefMaintenanceMoveNeighborIndex(index, -1) < 0;
  down.disabled = sorted || index < 0 || viewDefMaintenanceMoveNeighborIndex(index, 1) < 0;
  const sortNote = sorted ? '（ソート中は並び順編集できません）' : '';
  up.title = `選択中のViewDef項目を同一Section内で1行上へ移動${sortNote}`;
  down.title = `選択中のViewDef項目を同一Section内で1行下へ移動${sortNote}`;
}

function moveSelectedViewDefMaintenanceRow(delta) {
  if (typeof isViewDefMaintenanceDocument !== 'function' || !isViewDefMaintenanceDocument(sourceData)) {
    setStatus('ViewDefメンテナンス画面でのみ並び順を変更できます', { kind: 'warn', title: 'ViewDef並び替え' });
    return false;
  }
  if (sortState?.field) {
    setStatus('列ソート中はViewDefの並び順を変更できません。列見出しのソートを解除してから実行してください。', { kind: 'warn', title: 'ViewDef並び替え' });
    return false;
  }
  const rowKey = currentViewDefMaintenanceRowKey();
  if (!rowKey || typeof moveViewDefMaintenanceProjectionRow !== 'function') {
    setStatus('移動するViewDef項目を1行選択してください', { kind: 'warn', title: 'ViewDef並び替え' });
    return false;
  }

  const result = moveViewDefMaintenanceProjectionRow(sourceData, rowKey, Number(delta));
  if (!result?.moved) {
    const msg = result?.reason === 'section_boundary'
      ? 'このSectionではこれ以上移動できません'
      : 'ViewDef項目を移動できませんでした';
    setStatus(msg, { kind: 'warn', title: 'ViewDef並び替え' });
    updateViewDefMaintenanceMoveButtons();
    return false;
  }

  currentRows = getByPath(sourceData, gridDef().dataPath);
  selectedIndex = currentRows.findIndex(row => String(row?.__maintenance_key ?? '') === rowKey);
  applySearch();
  renderGrid();
  setStatus(
    Number(delta) < 0 ? 'ViewDef項目を1行上へ移動しました' : 'ViewDef項目を1行下へ移動しました',
    { kind: 'success', title: 'ViewDef並び替え', toast: false }
  );
  return true;
}
