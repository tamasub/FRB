...
async function loadStudioOverlayRuntime(options = {}) {
  ...
  const requestedId = overlaySafeId(options.overlayId) || DEFAULT_STUDIO_OVERLAY_ID;
  studioOverlayLoadPromise = (async () => {
    try {
      ...
      try {
        manifest = await fetchJson(`/api/overlays/${encodeURIComponent(requestedId)}/manifest`);
        source = `/api/overlays/${requestedId}/manifest`;
      } catch (apiErr) {
        try {
          manifest = await fetchJson(`studio_overlays/${encodeURIComponent(requestedId)}/studio_manifest.json`);
          source = `studio_overlays/${requestedId}/studio_manifest.json`;
        } catch {
          studioOverlayRuntime = {
            overlayId: requestedId,
            available: false,
            manifest: null,
            valueSetApiNames: [],
            viewDefApiNames: [],
          ...
          console.info(`Studio Overlay「${requestedId}」は未配置です`, apiErr);
          return studioOverlayRuntime;
        }
      }

      const runtime = normalizeOverlayManifest(manifest, requestedId);
